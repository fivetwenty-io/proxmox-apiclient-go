...
import (
	...
	"github.com/fivetwenty-io/proxmox-apiclient-go/v3/pkg/pbs/push"
)
...
func TestGenerated_Push_Methods(t *testing.T) {
	...
	t.Run("Push", func(t *testing.T) {
		...
		err := svc.Push(ctx, &push.PushParams{Remote: "sample-remote", RemoteStore: "sample-remote-store", Store: "sample-store"})
		if err != nil {
			t.Fatalf("Push: unexpected error: %v", err)
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "POST", "/api2/json/push")
		...
		if err := svc.Push(nilCtx, &push.PushParams{Remote: "sample-remote", RemoteStore: "sample-remote-store", Store: "sample-store"}); err == nil {
			t.Errorf("Push: expected error for nil context, got nil")
		}
	})
	t.Run("ErrorPath_POST", func(t *testing.T) {
		...
		err := svc.Push(ctx, &push.PushParams{Remote: "sample-remote", RemoteStore: "sample-remote-store", Store: "sample-store"})
		if err == nil {
			t.Fatalf("Push: expected error on 500 response, got nil")
		}
	})
}
