...
import (
	...
	"github.com/fivetwenty-io/proxmox-apiclient-go/v3/pkg/pbs/pull"
)
...
func TestGenerated_Pull_Methods(t *testing.T) {
	...
	t.Run("Pull", func(t *testing.T) {
		...
		err := svc.Pull(ctx, &pull.PullParams{RemoteStore: "sample-remote-store", Store: "sample-store"})
		if err != nil {
			t.Fatalf("Pull: unexpected error: %v", err)
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "POST", "/api2/json/pull")
		assertParamValue(t, got.form, "remote-store", "sample-remote-store")
		assertParamValue(t, got.form, "store", "sample-store")

		var nilCtx context.Context
		if err := svc.Pull(nilCtx, &pull.PullParams{RemoteStore: "sample-remote-store", Store: "sample-store"}); err == nil {
			t.Errorf("Pull: expected error for nil context, got nil")
		}
	})
	t.Run("ErrorPath_POST", func(t *testing.T) {
		...
		err := svc.Pull(ctx, &pull.PullParams{RemoteStore: "sample-remote-store", Store: "sample-store"})
		if err == nil {
			t.Fatalf("Pull: expected error on 500 response, got nil")
		}
	})
}
