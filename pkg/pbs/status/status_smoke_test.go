...
import (
	...
	"github.com/fivetwenty-io/proxmox-apiclient-go/v3/pkg/pbs/status"
)
...
func TestGenerated_Status_Methods(t *testing.T) {
	...
	t.Run("ListStatus", func(t *testing.T) {
		...
		err := svc.ListStatus(ctx)
		if err != nil {
			t.Fatalf("ListStatus: unexpected error: %v", err)
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/status")

		var nilCtx context.Context
		if err := svc.ListStatus(nilCtx); err == nil {
			t.Errorf("ListStatus: expected error for nil context, got nil")
		}
	})
	t.Run("ListDatastoreUsage", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":[],"success":1}`)

		resp, err := svc.ListDatastoreUsage(ctx)
		if err != nil {
			t.Fatalf("ListDatastoreUsage: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("ListDatastoreUsage: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/status/datastore-usage")

		var nilCtx context.Context
		if _, err := svc.ListDatastoreUsage(nilCtx); err == nil {
			t.Errorf("ListDatastoreUsage: expected error for nil context, got nil")
		}
	})
	t.Run("ListMetrics", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":{},"success":1}`)

		err := svc.ListMetrics(ctx, &status.ListMetricsParams{})
		if err != nil {
			t.Fatalf("ListMetrics: unexpected error: %v", err)
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/status/metrics")

		var nilCtx context.Context
		if err := svc.ListMetrics(nilCtx, &status.ListMetricsParams{}); err == nil {
	...
	t.Run("ErrorPath_GET", func(t *testing.T) {
		...
		err := svc.ListStatus(ctx)
		if err == nil {
			t.Fatalf("ListStatus: expected error on 500 response, got nil")
		}
	})
}
