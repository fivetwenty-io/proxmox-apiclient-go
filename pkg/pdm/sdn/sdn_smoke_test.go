// Code generated by cmd/pvegen. DO NOT EDIT.

package sdn_test

import (
	"context"
	"encoding/json"
	"net/http"
	"net/http/httptest"
	"net/url"
	"strconv"
	"sync"
	"testing"

	pveclient "github.com/fivetwenty-io/proxmox-apiclient-go/v3/pkg/client"
	"github.com/fivetwenty-io/proxmox-apiclient-go/v3/pkg/pdm/sdn"
)

var _ = json.RawMessage(nil)

func smokeOptsFromServerURL(u string) pveclient.Options {
	parsed, err := url.Parse(u)
	if err != nil {
		panic("test setup: invalid server URL: " + err.Error())
	}

	host := parsed.Hostname()

	port := 0
	if p := parsed.Port(); p != "" {
		port, _ = strconv.Atoi(p)
	}

	return pveclient.Options{
		Host:     host,
		Port:     port,
		Protocol: "http",
		APIToken: "user@pam!tok=sec",
	}
}

// recordedRequest captures what the mock server observed for the most
// recently dispatched request.
type recordedRequest struct {
	method string
	path   string
	query  url.Values
	form   url.Values
}

// testHarness runs a single httptest.Server shared by every subtest in this
// file. Subtests are never run in parallel (each sets the desired response
// immediately before invoking the method under test, then reads back the
// recorded request), so the mutex below only guards against the harness
// goroutine and the test goroutine overlapping on a single in-flight
// request, never against concurrent subtests.
type testHarness struct {
	mu       sync.Mutex
	last     recordedRequest
	nextCode int
	nextBody string
}

// newTestHarness starts the mock server and returns it alongside the harness
// used to configure responses and inspect recorded requests.
func newTestHarness() (*httptest.Server, *testHarness) {
	h := &testHarness{nextCode: http.StatusOK}

	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		h.mu.Lock()
		defer h.mu.Unlock()

		_ = r.ParseForm()
		h.last = recordedRequest{
			method: r.Method,
			path:   r.URL.Path,
			query:  r.URL.Query(),
			form:   r.PostForm,
		}

		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(h.nextCode)
		_, _ = w.Write([]byte(h.nextBody))
	}))

	return srv, h
}

// set configures the status code and body the next request will receive.
func (h *testHarness) set(code int, body string) {
	h.mu.Lock()
	defer h.mu.Unlock()

	h.nextCode = code
	h.nextBody = body
}

// snapshot returns a copy of the most recently recorded request.
func (h *testHarness) snapshot() recordedRequest {
	h.mu.Lock()
	defer h.mu.Unlock()

	return h.last
}

// assertRequestLine fails the test when the recorded method or resolved
// path does not exactly match what the endpoint's spec entry declares.
func assertRequestLine(t *testing.T, got recordedRequest, wantMethod, wantPath string) {
	t.Helper()

	if got.method != wantMethod {
		t.Errorf("method = %q, want %q", got.method, wantMethod)
	}

	if got.path != wantPath {
		t.Errorf("path = %q, want %q", got.path, wantPath)
	}
}

// assertParamValue fails the test when the wire-encoded value of key does
// not exactly equal want.
func assertParamValue(t *testing.T, values url.Values, key, want string) {
	t.Helper()

	if got := values.Get(key); got != want {
		t.Errorf("param %q = %q, want %q", key, got, want)
	}
}

// assertParamPresent fails the test when key is absent from values. Used for
// required parameters whose wire encoding is not a simple comparable string
// (e.g. json.RawMessage-typed fields).
func assertParamPresent(t *testing.T, values url.Values, key string) {
	t.Helper()

	if _, ok := values[key]; !ok {
		t.Errorf("param %q missing from request", key)
	}
}

func TestGenerated_Sdn_Methods(t *testing.T) {
	srv, harness := newTestHarness()
	defer srv.Close()

	c, err := pveclient.NewClient(smokeOptsFromServerURL(srv.URL))
	if err != nil {
		t.Fatalf("NewClient: %v", err)
	}

	svc := sdn.New(c)
	ctx := context.Background()

	t.Run("ListSdn", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":{},"success":1}`)

		err := svc.ListSdn(ctx)
		if err != nil {
			t.Fatalf("ListSdn: unexpected error: %v", err)
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/sdn")

		var nilCtx context.Context
		if err := svc.ListSdn(nilCtx); err == nil {
			t.Errorf("ListSdn: expected error for nil context, got nil")
		}
	})
	t.Run("ListControllers", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":[],"success":1}`)

		resp, err := svc.ListControllers(ctx, &sdn.ListControllersParams{})
		if err != nil {
			t.Fatalf("ListControllers: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("ListControllers: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/sdn/controllers")

		var nilCtx context.Context
		if _, err := svc.ListControllers(nilCtx, &sdn.ListControllersParams{}); err == nil {
			t.Errorf("ListControllers: expected error for nil context, got nil")
		}
	})
	t.Run("ListVnets", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":[],"success":1}`)

		resp, err := svc.ListVnets(ctx, &sdn.ListVnetsParams{})
		if err != nil {
			t.Fatalf("ListVnets: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("ListVnets: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/sdn/vnets")

		var nilCtx context.Context
		if _, err := svc.ListVnets(nilCtx, &sdn.ListVnetsParams{}); err == nil {
			t.Errorf("ListVnets: expected error for nil context, got nil")
		}
	})
	t.Run("CreateVnets", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":{},"success":1}`)

		resp, err := svc.CreateVnets(ctx, &sdn.CreateVnetsParams{Remotes: []json.RawMessage{json.RawMessage("{}")}, Vnet: "sample-vnet"})
		if err != nil {
			t.Fatalf("CreateVnets: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("CreateVnets: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "POST", "/api2/json/sdn/vnets")
		assertParamPresent(t, got.form, "remotes")
		assertParamValue(t, got.form, "vnet", "sample-vnet")

		var nilCtx context.Context
		if _, err := svc.CreateVnets(nilCtx, &sdn.CreateVnetsParams{Remotes: []json.RawMessage{json.RawMessage("{}")}, Vnet: "sample-vnet"}); err == nil {
			t.Errorf("CreateVnets: expected error for nil context, got nil")
		}
	})
	t.Run("ListZones", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":[],"success":1}`)

		resp, err := svc.ListZones(ctx, &sdn.ListZonesParams{})
		if err != nil {
			t.Fatalf("ListZones: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("ListZones: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "GET", "/api2/json/sdn/zones")

		var nilCtx context.Context
		if _, err := svc.ListZones(nilCtx, &sdn.ListZonesParams{}); err == nil {
			t.Errorf("ListZones: expected error for nil context, got nil")
		}
	})
	t.Run("CreateZones", func(t *testing.T) {
		harness.set(http.StatusOK, `{"data":{},"success":1}`)

		resp, err := svc.CreateZones(ctx, &sdn.CreateZonesParams{Remotes: []json.RawMessage{json.RawMessage("{}")}, Zone: "sample-zone"})
		if err != nil {
			t.Fatalf("CreateZones: unexpected error: %v", err)
		}
		if resp == nil {
			t.Fatal("CreateZones: response is nil")
		}

		got := harness.snapshot()
		assertRequestLine(t, got, "POST", "/api2/json/sdn/zones")
		assertParamPresent(t, got.form, "remotes")
		assertParamValue(t, got.form, "zone", "sample-zone")

		var nilCtx context.Context
		if _, err := svc.CreateZones(nilCtx, &sdn.CreateZonesParams{Remotes: []json.RawMessage{json.RawMessage("{}")}, Zone: "sample-zone"}); err == nil {
			t.Errorf("CreateZones: expected error for nil context, got nil")
		}
	})
	t.Run("ErrorPath_GET", func(t *testing.T) {
		harness.set(http.StatusInternalServerError, `{"data":null,"success":0,"message":"boom"}`)

		err := svc.ListSdn(ctx)
		if err == nil {
			t.Fatalf("ListSdn: expected error on 500 response, got nil")
		}
	})
	t.Run("ErrorPath_POST", func(t *testing.T) {
		harness.set(http.StatusInternalServerError, `{"data":null,"success":0,"message":"boom"}`)

		_, err := svc.CreateVnets(ctx, &sdn.CreateVnetsParams{Remotes: []json.RawMessage{json.RawMessage("{}")}, Vnet: "sample-vnet"})
		if err == nil {
			t.Fatalf("CreateVnets: expected error on 500 response, got nil")
		}
	})
}
